...
def translate_with_migration(transformer, de_sentence, client):
    ...
    try:
        ...
        while len(en_token_ids) < SEQ_MAX_LEN:
            ...
            if not migration_sent and client.should_migrate(start_time):
                ...
                for i, layer in enumerate(transformer.decoder.decoder_blocks):
                    ...
                    client.initial_kv_positions[layer_key] = {
                        'self_attn_K': layer.self_attn.kv_cache['K'].shape[1],
                        'self_attn_V': layer.self_attn.kv_cache['V'].shape[1],
                    }
                    initial_kv_cache[layer_key] = {
                        'self_attn': {
                            'K': layer.self_attn.kv_cache['K'].clone().cpu(),
                            'V': layer.self_attn.kv_cache['V'].clone().cpu()
                        }
                    }
                print(f"发送 KV 缓存形状: {initial_kv_cache[layer_key]['self_attn']['K'].shape}")
                print(f"发送 KV 缓存形状: {initial_kv_cache[layer_key]['self_attn']['V'].shape}")
            ...
            if client.received_ack is True:
                print(dec_x_batch)
            ...
            if migration_sent and client.received_ack:
                ...
                for i, layer in enumerate(transformer.decoder.decoder_blocks):
                    ...
                    delta_kv_cache[layer_key] = {
                        'self_attn': {
                            'K': layer.self_attn.kv_cache['K'][:, initial_pos['self_attn_K']:, :].clone().cpu(),
                            'V': layer.self_attn.kv_cache['V'][:, initial_pos['self_attn_V']:, :].clone().cpu()
                        },
                    }
...
if __name__ == '__main__':
    ...
    result = translate_with_migration(
        transformer, 
         'Heute scheint die Sonne hell und warm am blauen Himmel, während der Wind sanft durch die grünen Bäume weht. Heute scheint die Sonne hell und warm am blauen Himmel, während der Wind sanft durch die grünen Bäume weht.',
       client
    )
    end_time1=time.time()   
    print(f"[HostB] 数据迁移时间: {end_time1 - start_time1:.6f} 秒")
    text =  'Heute scheint die Sonne hell und warm am blauen Himmel, während der Wind sanft durch die grünen Bäume weht. Heute scheint die Sonne hell und warm am blauen Himmel, während der Wind sanft durch die grünen Bäume weht.'
