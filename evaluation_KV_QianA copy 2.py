...
def calculate_kv_cache_size(kv_cache):
      """ 计算 KVCache 的大小，单位为 GB """
      total_size = 0
      for layer_key, layer_data in kv_cache.items():
        for attention_type in ['self_attn', 'cross_attn']:
            K = layer_data[attention_type]['K']
            V = layer_data[attention_type]['V']
            batch_size, sequence_length, d_model = K.shape
            batch_size1, sequence_length1, d_model1 = V.shape
            # 每个 K 和 V 的 shape 为 (batch_size, sequence_length, d_model)
            # 计算每个矩阵的内存占用量 (batch_size * sequence_length * d_model * 4 字节)
            total_size += K.numel() * 4 / (1024 ** 2)  # 转换为 GB
            total_size += V.numel() * 4 / (1024 ** 2)  # 转换为 GB
           # print(f"{ K.numel() },{V.numel()}")
           # print(f"{batch_size},{sequence_length},{d_model}")
           # print(f"{batch_size1},{sequence_length1},{d_model1}")            
      return total_size

def calculate_model_size(model):
    model_size = 0
    for param in model.parameters():
        model_size += param.numel() * param.element_size()  # numel() 是参数的元素个数，element_size() 是每个元素的字节数
    return model_size / (1024 ** 2)  # 转换为MB
...
def translate_with_migration(transformer, de_sentence, client, page_id):
    ...
    batch_size = len(de_sentence)
    # 动态判断请求类型
    
    is_long_request = len(de_tokens) > 5 # 假设50个token为阈值
...
def handle_long_request(transformer, enc_x, encoder_z, de_tokens, client, start_time, page_id):  
    ...
    try:
        ...
        while len(en_token_ids) < SEQ_MAX_LEN  :
            ...
            if not migration_sent and client.should_migrate(start_time):
               ...
               # print(f"2")
                ...
                for i, layer in enumerate(transformer.decoder.decoder_blocks):
                  ...
                  #  print(f"{layer.second_multihead_attn.get_kv(page_id, layer_id=i)['K'].shape[1] }")
                  #  print(f"{layer.second_multihead_attn.get_kv(page_id, layer_id=i)['V'].shape[1] }")
                    ...
                    client.initial_kv_positions[layer_key] = {    
                        ...
                        'cross_attn_K': layer.second_multihead_attn.get_kv(page_id, layer_id=i)['K'].shape[1],
                        'cross_attn_V': layer.second_multihead_attn.get_kv(page_id, layer_id=i)['V'].shape[1]
                    }
                ...
                print(f"生成的初始 KV 缓存总大小: {calculate_kv_cache_size(initial_kv_cache):.4f} MB") 
            ...
            if migration_sent and client.received_ack:
                ...
                for i, layer in enumerate(transformer.decoder.decoder_blocks):
                   ...
                   # print(f"{initial_pos}")
                    ...
                    #print(f"生成的初始 KV 缓存总大小: {initial_kv_cache},{calculate_kv_cache_size(initial_kv_cache):.4f} GB") 
                print(f"增量 KV 缓存总大小: {calculate_kv_cache_size(delta_kv_cache):.4f} MB")                        
          ...
          #  print(f"生成的初始 KV 缓存总大小: {calculate_kv_cache_size(initial_kv_cache):.4f} GB") 
            time.sleep(0.5)
...
if __name__ == '__main__':
  ...
  model_size_mb = calculate_model_size(transformer)
  print(f"模型的大小: {model_size_mb:.2f} MB")
  end_time1=time.time()   
...
'''
  ...
  with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
    for num_requests in range(1, 2):  # 测试1到20个短请求的负载
