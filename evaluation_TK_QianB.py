...
class HybridMigrationServer:
    def __init__(self, model_path):
        self.model = Transformer(
            enc_vocab_size=len(de_vocab),
            dec_vocab_size=len(en_vocab),
            emb_size=512, q_k_size=64, v_size=64,
            f_size=2048, head=8, nblocks=6,
            dropout=0.1, seq_max_len=SEQ_MAX_LEN
        ).to(DEVICE)
    ...
    def hybrid_continue(self, data):
        ...
        # 重建前部分KV
        for i in range(1, len(token_part)):
        ...
        # 阶段3：继续生成
        current_seq = token_part + [0] * kv_data['start_idx']  # 占位符
        return self._generate_remaining(enc_x, current_seq)
    ...
    def _generate_remaining(self, enc_x, en_token_ids):
        """生成剩余Token"""
        try:
            encoder_z = self.model.encode(enc_x)
            ...
            while len(en_token_ids) < SEQ_MAX_LEN:
                ...
                with torch.no_grad():
                    decoder_z = self.model.decode(dec_input, encoder_z, enc_x)
                
                next_id = torch.argmax(decoder_z[0, -1, :]).item()
                en_token_ids.append(next_id)
                if next_id == EOS_IDX:
                    break
